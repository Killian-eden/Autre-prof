...
let Orb = function( x, y ){
    let dx = ( x / opt.scale ) - ( center.x / opt.scale ),
...
let createOrb = function( config ){
    let x = ( config && config.x ) ? config.x : sketch.mouse.x,
        y = ( config && config.y ) ? config.y : sketch.mouse.y;
    orbs.push( new Orb( x, y ) );
};

let turnOnMove = function(){
    sketch.mousemove = createOrb;
};

let turnOffMove = function(){
